...
* Clase - busqueda_fichas_reserva
*/

class busqueda_fichas_reserva extends CI_Controller {
    
    function __construct() {
        parent::__construct();
        $this->load->helper('url');
        $this->load->model('busqueda_fichas_reserva_model');
    ...
    function index() {
        ...
        $this->load->view("busqueda_fichas_reserva_view");
    }
    ...
    function consulta($param="",$cantReg=30) {   
        ...
        $result = $this->busqueda_fichas_reserva_model->consulta_db($param, $cantReg, $condicion, $order);
        ...
        $config['base_url'] = site_url("busqueda_fichas_reserva/consulta");
        $config['total_rows'] = $this->busqueda_fichas_reserva_model->cantidadRegistros($condicion);
    ...
    function armoImpresion(){   
        ...
        }else if( $this->busqueda_fichas_reserva_model->cantidadRegistros($condicion) < (($a_pagina * 30) - 30) ){
            echo "No existe tal cantidad de paginas para esa consulta verifique";
        }else{
            echo "1";
            if( $this->busqueda_fichas_reserva_model->cantidadRegistros($condicion) <= 30 ){
                $ini   = 0;
                $param = 30;
