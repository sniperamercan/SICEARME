...
class listado_actas_baja extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('listado_actas_baja_model');
    ...
    function index() {
        ...
        $this->load->view("listado_actas_baja_view");
    }
    ...
    function consulta($param="",$cantReg=30) {   
        ...
        $result = $this->listado_actas_baja_model->consulta_db($param, $cantReg, $condicion, $order);
        ...
        for($i=0;$i<count($result);$i=$i+8) {
            ...
            */
            ...
            $unidad_entrega = $this->listado_actas_baja_model->nombreUnidad($result[$i+2]);
            $unidad_recibe  = $this->listado_actas_baja_model->nombreUnidad($result[$i+3]);
        ...
        $config['base_url'] = site_url("listado_actas_baja/consulta");
        $config['total_rows'] = $this->listado_actas_baja_model->cantidadRegistros($condicion);
    ...
    function armoImpresion(){   
        ...
        }else if( $this->listado_actas_baja_model->cantidadRegistros($condicion) < (($a_pagina * 30) - 30) ){
            echo "No existe tal cantidad de paginas para esa consulta verifique";
        }else{
            echo "1";
            if( $this->listado_actas_baja_model->cantidadRegistros($condicion) <= 30 ){
                $ini   = 0;
                $param = 30;
    ...
    function verObservaciones() {
        ...
        $observaciones = $this->listado_actas_baja_model->verObservaciones($nro_acta);
    ...
    function verEntregas() {
        ...
        if(!$this->listado_actas_baja_model->tieneFichas($nro_acta)) {
            echo "El nro de acta - ".$nro_acta." no tiene ninguna ficha asociada";
        }else {
            
            $fichas = array();
            $fichas = $this->listado_actas_baja_model->verFichas($nro_acta);
            
            $accesorios = array();
            $accesorios = $this->listado_actas_baja_model->verAccesorios($nro_acta);
