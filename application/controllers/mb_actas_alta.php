...
class mb_actas_alta extends CI_Controller {
    ...
    function consulta($param="",$cantReg=30) {   
        ...
        $config['total_rows'] = $this->mb_actas_alta_model->cantidadRegistros($condicion);
    ...
    function armoImpresion(){   
        ...
        }else if( $this->mb_actas_alta_model->cantidadRegistros($condicion) < (($a_pagina * 30) - 30) ){
            echo "No existe tal cantidad de paginas para esa consulta verifique";
        }else{
            echo "1";
            if( $this->mb_actas_alta_model->cantidadRegistros($condicion) <= 30 ){
                $ini   = 0;
                $param = 30;
