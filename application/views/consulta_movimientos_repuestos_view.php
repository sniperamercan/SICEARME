    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function filtrarMovimientos(){
                ...
                $.ajax({ 
                    type: 'post',
                    url: '<?php echo base_url(); ?>consulta_movimientos_repuestos/consulta/0',
                    data: "nro_catalogo="+nro_catalogo,
                    success: function(){
                        cargoConsultaMovimientos();
                    }
                });                
            }
            ...
            function seteoImpresion(de_pagina, a_pagina){                
                $.ajax({
                    type: 'post',
                    url: "<?php echo base_url("consulta_movimientos_repuestos/armoImpresion"); ?>",
            ...
            function orderBy(param){            
                $.ajax({
                    type: 'post',
                    url: "<?php echo base_url("consulta_movimientos_repuestos/orderBy"); ?>",
                    data: "order="+param,
                    success: function(){
                        cargoConsulta();                       
                    }                  
            ...
            function cargoConsultaMovimientos() {
                $.ajax({
                    type: 'post',
                    dataType: 'json',
                    url: "<?php echo base_url("consulta_movimientos_repuestos/consulta"); ?>",
                    success: function(data){
                        if(data[0] != 0) {
                            $("#datos_consulta").html(data[0]);
                        }else {
                            jAlert("ERROR: Debe seleccionar un nro de catalogo para realizar dicha consulta", "Error");
                        }
            ...
            function cargoCatalogosFiltro() {
                $.ajax({
                   type: "post",
                   url: "<?php base_url(); ?>consulta_movimientos_repuestos/cargoCatalogosFiltro",
                   success: function(data) {
                       $("#nro_catalogo").val("");
