...
class imprimir_ordenes_trabajo_model extends CI_Model {
    ...
    function consulta_db($nro_orden){
        ...
        $query = $this->db->query("SELECT fecha, nro_serie, marca, calibre, modelo, nombreunidad, estado_orden_trabajo, estado_arma
                                   FROM ordenes_trabajo o
                                   INNER JOIN unidades u ON u.idunidad = o.idunidad
                                   WHERE (estado_orden_trabajo = 0 OR estado_orden_trabajo = 1)
                                   AND nro_orden = ".$this->db->escape($nro_orden));
    ...
    function consulta_db_acciones($nro_orden) {
